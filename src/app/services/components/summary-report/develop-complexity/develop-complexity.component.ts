...
@Component({
  selector: 'tecno-develop-complexity',
  templateUrl: './develop-complexity.component.html',
  styleUrls: ['./develop-complexity.component.scss']
})
export class DevelopComplexityComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log("developComp", this.data);
  }
  ...
  fillChart(selected: number){
    ...
    for (let index = 0; index < template_req.item_severities.length; index++) {
      this.colorScheme.domain.push(this.getColor(template_req.item_severities[index].item_severity.color));
      this.chartData.push(
        {
          name: template_req.item_severities[index].item_severity.title,
          value: template_req.item_severities[index].percent
        }
      );
    }
  }
  ...
  onFilterDateEvent(rangeDate, graph: string) {
    ...
    this._totalItemsGraphService.getTotalItems(objDate)
      .subscribe(data => {
        this.data = data.develop_complexity;
        this.fillChart(this.template_id);
      });
