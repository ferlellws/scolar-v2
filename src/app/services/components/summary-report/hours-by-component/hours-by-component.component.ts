...
@Component({
  selector: 'tecno-hours-by-component',
  templateUrl: './hours-by-component.component.html',
  styleUrls: ['./hours-by-component.component.scss']
})
export class HoursByComponentComponent implements OnInit {


  @Input() data: Array<any>;
  @Input() template_id: number;
  columns_labels: string[] = ['Aplicación'];
  columns_footers: string[] = ['Total'];
  displayedColumns: string[] = [ 'component_label'];
  ...
  fillTable(selected: number){    
    ...
    var data = [];
    for (let index = 0; index < template_data.components.length; index++) {
      //llenado de columnas
      if(index == 0){
        var severities: any[] = template_data.components[index].item_severities_component;
      ...
      var row: any = {};
      row[this.displayedColumns[0]] = template_data.components[index].component.title;
      for (let j = 1; j < this.displayedColumns.length - 1; j++) {
        var hours_severity = template_data.components[index].item_severities_component.filter(item_severity=> item_severity.severity.id == parseInt(this.displayedColumns[j]))[0].hours_severity;
        row[this.displayedColumns[j]] = `${hours_severity.hours}:${this.formatMinutes(hours_severity.minutes)}`;
      }
      row[this.displayedColumns[this.displayedColumns.length - 1]] = `${template_data.components[index].hours_component.hours}:${this.formatMinutes(template_data.components[index].hours_component.minutes)}`;
      data.push(row);
    }
  ...
  onFilterDateEvent(rangeDate, graph: string) {
    ...
    this._indicaatorsHoursGraphService.getHoursItems(objDate)
      .subscribe(data => {
        this.data = data.hours_by_component;
        this.fillTable(this.template_id);
      });
